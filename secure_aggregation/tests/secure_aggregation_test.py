...
class SecureAggregatorTest(unittest.TestCase):
    ...
    def test_fixedpoint_init(self):
        ...
        self.assertEqual(converter.max_value, 32767)
        self.assertEqual(converter.min_value, -32768)
    ...
    def test_floating_to_fixedpoint(self):
        ...
        self.assertEqual(y, torch.tensor(32767))

        x = torch.tensor(-327.69)
        y = converter.to_fixedpoint(x)
        # y = x * scaling_factor = -32769 ==> adjust to minValue -32768
        self.assertEqual(y, torch.tensor(-32768))
    ...
    def test_fixed_to_floating_point(self):
        ...
        self.assertTrue(torch.allclose(y, torch.tensor(1.0), rtol=1e-10))

        x = torch.tensor(157)
        y = converter.to_float(x)
        # y = x / scaling_factor = 1.847058823529412
        self.assertTrue(torch.allclose(y, torch.tensor(1.847058823529412), rtol=1e-10))
    ...
    def test_params_floating_to_fixedpoint_different_config_for_layers(self):
        ...
        for name, p in model.named_parameters():
            if name == "fc1.weight" or name == "fc1.bias":
                # round 547.28 to 547
                self.assertTrue(torch.allclose(p, torch.tensor(547.0), rtol=1e-10))
            if name == "fc2.weight" or name == "fc2.bias":
                # round 54.728 to 55
                self.assertTrue(torch.allclose(p, torch.tensor(55.0), rtol=1e-10))
    ...
    def test_params_fixed_to_floating_point_different_config_for_layers(self):
        ...
        for name, p in model.named_parameters():
            if name == "fc1.weight" or name == "fc1.bias":
                # 832.8 / 30 = 27.76
                self.assertTrue(torch.allclose(p, torch.tensor(27.76), rtol=1e-10))
            if name == "fc2.weight" or name == "fc2.bias":
                # 832.8 / 80 = 10.41
                self.assertTrue(torch.allclose(p, torch.tensor(10.41), rtol=1e-10))
