...
"""Algrorithm 2: SARAH: Periodic variance reduction with inexact proximal minimization
"""
...
class SarahServer(ISyncServer):
    def __init__(
        ...
        **kwargs,
        ...
        self.gt = deepcopy(global_model.fl_get_module())
    ...
    def select_clients_for_training(
        ...
        """
        if (self._round_number) % self.cfg.large_cohort_period == 0:
    ...
    def receive_update_from_client(self, message: Message):
        ...
        # p * d_{i}^{t} where p = client's # of examples
    ...
    def step(self):
        ...
        averaged_client_updates = self._aggregator.aggregate()
        if (self._round_number) % self.cfg.large_cohort_period == 0:
            FLModelParamUtils.set_gradient(
                model=self._global_model.fl_get_module(),
                reference_gradient=averaged_client_updates,
            )
            self.gt = deepcopy(averaged_client_updates)
        else:
            # g^t = g^{t-1} + 1/m \sigma_{i \in S_t} (d_{i}^{t})
            FLModelParamUtils.add_model(
                model1=self.gt,
                model2=averaged_client_updates,
                model_to_save=averaged_client_updates,
            )
