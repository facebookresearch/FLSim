...
class JobSchedulerUtilTest(testutil.BaseFacebookTestCase):
    def setUp(self) -> None:
        super().setUp()
    ...
    def test_test_async_job_scheduler(self) -> None:
        ...
        self.assertEqual(len(model_trainer.num_unseen_global_model_updates), 0)
        self.assertEqual(len(model_trainer.training_events), 0)
        ...
        for user_index in range(num_users):
            ...
            self.assertEqual(client.model_seqnum, model_trainer.current_seqnum)
            self.assertEqual(client.model_seqnum, user_index)
            model_trainer.on_training_end(client)
        # each user generates two events: one TrainingFinished, and one TrainingStart
        self.assertEqual(len(model_trainer.training_events), 2 * num_users)
        # training proceeds sequentially, so num_unseen_global_model_updates should
        # be a list of zeroes
        self.assertEqual(model_trainer.num_unseen_global_model_updates, [0] * num_users)
