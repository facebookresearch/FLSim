...
class TimelineTest(unittest.TestCase):
    def test_global_round_num(self):
        ...
        self.assertEqual(tl.global_round_num(), 1)
        ...
        self.assertEqual(tl.global_round_num(), 7)
        ...
        self.assertEqual(tl.global_round_num(), tl.global_round)

    def test_as_float(self):
        # test first round
        tl = Timeline(epoch=1, round=1)
        self.assertAlmostEqual(tl.as_float(), 1.0)  # float starts from 1

        # test first round
        tl = Timeline(epoch=1, round=1, rounds_per_epoch=100)
        self.assertAlmostEqual(tl.as_float(), 1 / 100)

        # first round of 10th epoch
        tl = Timeline(epoch=10, round=1)
        self.assertAlmostEqual(tl.as_float(), 10.0)

        # test first round does not depned on rounds_per_epoch
        tl = Timeline(epoch=10, round=1, rounds_per_epoch=100)
        self.assertAlmostEqual(tl.as_float(), 9 + (1 / 100))

        # test offset
        tl = Timeline(epoch=2, round=1, rounds_per_epoch=100)
        self.assertAlmostEqual(tl.as_float(-1), 1.0)

        # test from global_round
        tl = Timeline(global_round=12, rounds_per_epoch=10)
        self.assertAlmostEqual(tl.as_float(), 1.2)
        self.assertAlmostEqual(tl.as_float(1), 1.3)
    ...
    def test_tick_simple(self):
        ...
        # test every 2 epoch
        tl = Timeline(epoch=1, round=1, rounds_per_epoch=10)
        self.assertFalse(tl.tick(tick_interval=2))
        ...
        tl = Timeline(epoch=2, round=10, rounds_per_epoch=10)
        self.assertTrue(tl.tick(tick_interval=2))

        # test every 0.2 epoch
        tl = Timeline(epoch=1, round=1, rounds_per_epoch=10)
        self.assertFalse(tl.tick(tick_interval=0.2))

        tl = Timeline(epoch=1, round=2, rounds_per_epoch=10)
        self.assertTrue(tl.tick(tick_interval=0.2))
        ...
        self.assertFalse(tl.tick(tick_interval=3))
        self.assertTrue(tl.tick(tick_interval=2))
